...
export const BentoGridItem = ({
  ...
  const handleCopy = () => {
    const text = "luke.sousa.dev@gmail.com";
    ...
    >
      <div className={`${id === 6 && "flex justify-center"} h-full`}>
        ...
        >
          ...
          <div
            className={`z-10 max-w-96 font-sans text-lg font-bold lg:text-2xl`}
          >
            {title}
          </div>
            ...
            <div className="absolute -right-3 flex w-fit gap-1 lg:-right-2 lg:gap-5">
              <div className="flex flex-col gap-3 md:gap-3 lg:gap-8">
                {leftLists.map((item, i) => (
                  <span
                    key={i}
                    className="rounded-lg bg-[#10132E] px-3 py-2 text-center text-xs opacity-50 lg:px-3 lg:py-4 lg:text-sm lg:opacity-100"
                  >
                    {item}
                  </span>
                ))}
